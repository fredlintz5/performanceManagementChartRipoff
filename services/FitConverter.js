 ...
 *
*/
function parseFitFile(filePath, cb) {
...
function getAllFlattenedRecords(records = []) {
  return records.filter(({ type }) => type === 'record').map(({ data }) => ({ ...data }))
}

function getPerLapWorkoutDataFromChunks(chunkedRecords = []) {
  return chunkedRecords.reduce((acc, lap, index) => ({ ...acc, [`Lap ${index + 1}`]: getWorkoutData(lap) }), {})
}

function getTotalWorkoutDataFromChunks(chunkedRecords = []) {
  return getWorkoutData(chunkedRecords.reduce((acc, lap) => acc.length ? [...acc, ...lap] : [...lap], []))
}

function getChunkedLapRecords(records = []) {
  let nestedArray = []
  /*
